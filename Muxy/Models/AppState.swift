...
final class AppState {
    ...
    func restoreSelection(projects: [Project], worktrees: [UUID: [Worktree]]) {
        ...
        // Muxy fork (CCH parity): do NOT reuse the snapshot's terminal panes — that
        // path bypasses ClaudeSessionStore and starts fresh Claude sessions (losing
        // conversation resume). Instead, for each restored worktree we rebuild a
        // fresh TabArea via TabArea(projectPath:), which loads the persisted Claude
        // session list and resumes each conversation by its stable id (--resume).
        // We still honor the snapshot only to know WHICH worktrees were open.
        let worktreePathByID: [UUID: String] = worktrees.values
            .flatMap { $0 }
            .reduce(into: [:]) { $0[$1.id] = $1.path }
        for entry in restored {
            guard let path = worktreePathByID[entry.key.worktreeID] else { continue }
            let area = TabArea(projectPath: path)
            workspaceRoots[entry.key] = .tabArea(area)
            focusedAreaID[entry.key] = area.id
        }
